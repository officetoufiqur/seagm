...
const data = ref(props.carousels);


function deleteCarousel(id: number) {
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
            router.delete(`/carousels/destroy/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    data.value = props.carousels;
                }
            });
        }
    })
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="Carousels List" create-btn create-text="Create"
                create-url="/carousels/create">
                ...
                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/carousels/edit/${item.id}`"
