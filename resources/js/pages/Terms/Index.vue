...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="Terms and Conditions List" create-btn create-text="Create Term"
                create-url="/terms/create">

                <template #content="{ item }">
                   <span>{{ item.content ? item.content.substring(0, 120) + '...' : item.content }}</span>
                </template>

                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/terms/edit/${item.id}`"
                        ...
                        <button @click="deleteTerms(item.id)"
