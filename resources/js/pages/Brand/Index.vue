...
function deleteCard(id: number) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(`/brands/destroy/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    data.value = props.brands;
                }
            });
        }
    })
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="Brands List" :create-btn="true" create-text="Create"
                create-url="/brands/create">

                <template #logo="{ item }">
                    <img :src="item.logo" :alt="item.description" class="w-10 h-10 object-cover rounded" />
                </template>

                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/brands/edit/${item.id}`"
