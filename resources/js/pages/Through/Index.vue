...
function deleteCard(id: number) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(`/through/destroy/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    data.value = props.throughs;
                }
            });
        }
    })
...
<template>
    ...
    <Head title="Through Items" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <FlashMessage :message="props.flash.message" />
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="Through Items List" :create-btn="true" create-text="Create"
                create-url="/through/create">
                ...
                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/through/edit/${item.id}`"
