...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="News Category List" create-btn create-text="Create Category"
                create-url="/news-categories/create">

                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/news-categories/edit/${item.id}`"
