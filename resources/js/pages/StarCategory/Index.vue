...
function deleteCategory(id: number) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(`/star-category/destroy/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="Star category List" create-btn create-text="Create"
                create-url="/star-category/create">

                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/star-category/edit/${item.id}`"
