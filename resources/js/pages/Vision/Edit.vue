...
const form = useForm({
    id: props.vision.id,
    description: props.vision.description || '',
    image: null as File | null,

    items: props.vision.items?.length
        ? props.vision.items.map(item => ({
            id: item.id,
            title: item.title,
...
const submit = () => {
    form.post(`/vision/update/${props.vision.id}`, {
        forceFormData: true,
    });
};
...
const initDropify = () => {
    $('.mainImage').dropify({
        defaultFile: props.vision.image,
        height: 150,
    });
};
...
<template>
    ...
    <Head title="Vision Edit" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-white m-7 p-5 rounded">
            <!-- Header -->
            <div class="flex items-center justify-between border-b pb-3">
                <h1 class="text-xl font-medium">Vision Update</h1>
                <LinkButton label="Back" url="/vision" />
            </div>
            ...
            <div class="p-5">
                <form @submit.prevent="submit">
                    ...
                    <!-- Add Item -->
                    <button type="button" class="bg-blue-500 text-white px-4 py-1.5 rounded mb-4 text-sm ml-auto block mt-25"
                        @click="addItem">
                        + Add Card Item
                    </button>
