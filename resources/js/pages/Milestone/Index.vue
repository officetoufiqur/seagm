...
const data = ref(props.milestones);
...
function deleteCard(id: number) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(`/milestones/destroy/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    data.value = props.milestones;
                }
            });
        }
    })
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-7">
            <FilterTable :rows="data" :columns="columns" title="milestones List" :create-btn="true" create-text="Create"
                create-url="/milestones/create">

                <template #action="{ item }">
                    <div class="flex items-center gap-2">
                        <Link :href="`/milestones/edit/${item.id}`"
